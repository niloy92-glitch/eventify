...
def client_chat_view(request, conversation_id=None):
    ...
    conversations = list(
        Conversation.objects.filter(client=request.user).order_by(
            "-updated_at"
        )
    )
    ...
    if conversation_id:
        active_conversation = get_object_or_404(
            Conversation, pk=conversation_id, client=request.user
        )
        ...
        unread_messages = active_conversation.messages.filter(
            is_read=False
        ...
        if request.method == "POST":
            ...
            if content and not active_conversation.is_locked:
                ...
                return redirect(
                    reverse(
                        "chat:client_chat_detail",
                        args=[active_conversation.pk],
                    )
                )
...
def vendor_chat_view(request, conversation_id=None):
    ...
    conversations = list(
        Conversation.objects.filter(vendor=request.user).order_by(
            "-updated_at"
        )
    )
    ...
    if conversation_id:
        active_conversation = get_object_or_404(
            Conversation, pk=conversation_id, vendor=request.user
        )
        chat_messages = active_conversation.messages.all()
        active_bookings = active_conversation.active_bookings()
